...
# Reuse existing nodebuilder-outline container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-outline; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-outline -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:15.5-alpine3.19 docker.io/redis:7 docker.io/outlinewiki/outline:0.78.0-0" \
    "${container}"
# Commit the image
